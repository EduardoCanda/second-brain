...
Cliente de linha de comando para HTTP/HTTPS e outros protocolos. Resolve testes de API, headers, autenticação e timing de requisições.
...
curl -v --connect-timeout 3 https://api.exemplo.com/health
```
...
curl vs wget: curl é ideal para APIs e headers; wget é forte em downloads recorrentes.
