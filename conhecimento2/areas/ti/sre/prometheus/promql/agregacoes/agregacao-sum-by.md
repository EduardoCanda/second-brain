...
- **`by (...)`** (ou **`without (...)`**) é um **modificador do agregador** que diz **como agrupar** as séries antes de somar.
...
1. A expressão interna (ex.: `rate(...)`) produz **várias séries**, cada uma com um conjunto de labels.
...
- **Faça [[funcao-rate|rate()]] primeiro, agregue depois.** O `rate()` precisa operar **por série** para corrigir resets.
